...
public:
  ...
  template <typename... Args> IntervalT(Args... params) { Set(params...); }
...
template <typename T> inline T Dist(const IntervalT<T> &intvl, const T val) {
...
public:
  ...
  template <typename... Args> BoxT(Args... params) { Set(params...); }
...
template <typename T> inline T Dist(const BoxT<T> &box1, const BoxT<T> &box2) {
...
template <typename T> class SegmentT : public BoxT<T> {
public:
  using BoxT<T>::BoxT;
